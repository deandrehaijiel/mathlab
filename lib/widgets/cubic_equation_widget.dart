...
class CubicEquationWidget extends StatefulWidget {
  const CubicEquationWidget({super.key});

  @override
  State<CubicEquationWidget> createState() => _CubicEquationWidgetState();
}

class _CubicEquationWidgetState extends State<CubicEquationWidget> {
  ...
  final TextEditingController _dController = TextEditingController(text: '-30');

  num? discriminant;
  Map<String, Complex>? valuesOfX;

  void calculate() {
    CubicEquation q = CubicEquation(
      a: num.tryParse(_aController.text) ?? 0,
      b: num.tryParse(_bController.text) ?? 0,
      c: num.tryParse(_cController.text) ?? 0,
      d: num.tryParse(_dController.text) ?? 0,
    );
